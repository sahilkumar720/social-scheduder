    ...
    <div className="max-w-4xl mx-auto space-y-12 pb-20 animate-in fade-in duration-700">
      ...
      <div className="space-y-6 text-center mt-20">
        ...
        />
          <div className="absolute bottom-4 right-2.5 flex items-center gap-3 text-sm">
            ...
            className="flex items-center gap-3 bg-light-back py-2 px-3 rounded-lg">
              <span>
                AI Image
              
              </span>
              <div className={`relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full transition-colors duration-200 ease-in-out focus:outline-none ${generateImage ? "bg-accent" : "bg-slate-200"}`}>
            ...
            disabled = {loading}
            className="bg-slate-900 hover:bg-slate-800 text-white flex items-center gap-2 px-3 py-2 rounded-lg"
            >
              {loading ? (
                <>
     ...
     <div className="flex flex-wrap justify-center gap-2">
      {tones.map((t)=>(
        ...
        className={`px-4 py-1.5 rounded-full text-sm transition-all border ${tone === t ? "bg-accent border-brand text-white" : "bg-white border-slate-200 text-slate-500 hover:border-slate-300" }`}
        >
          {t}
        </button>
      ))}
            ...
            <div key={gen._id} className="group bg-white rounded-2xl border border-slate-100 p-5 hover:border-red-200 transition-all relative overflow-hidden">
              <div className="flex flex-col h-full space-y-4">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-green-600 bg-green-50 px-2 py-0.5 rounded-md " >{gen.tone}</span>
                </div>
                ...
                <p className="text-sm text-slate-600 line-clamp-3 leading-relaxed flex-1">{gen.content}
                  ...
                  <div className="flex items-center gap-2 pt-2">
                    ...
                    className="flex-1 bg-slate-100 hover:bg-accent hover:text-white text-slate-600 text-xs py-2.5 rounded-lg transition-all">
                      Schedule Post
        ...
        <div className="fixed inset-0 min-h-screen z-50 flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-md animate-in fade-in duration-300">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl border border-slate-100 overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-4">
              <div className="bg-slate-50 rounded-2xl p-6 border-slate-100 space-y-4 ">
            ...
            <div className="space-y-6">
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {PLATFORMS.map((p)=>{
                    ...
                    return(
                      <button key={p.id} onClick={()=>setSelectedPlatforms((prev)=> (prev.includes(p.id) ? prev.filter((x)=> x !== p.id) : [...prev, p.id]))} className={`p-2.5 rounded-md border text-xs ${active ? "bg-accent/80 text-white" : "bg-white border-slate-200 text-slate-400 hover:border-slate-300"}`}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="relative">
                  ...
                  <input type="date" className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-md text-slate-900 text-sm focus:outline-none transition-all" value={scheduledDate} onChange={(e)=> setScheduledDate(e.target.value)}/>
                ...
                <div className="relative">
                  ...
                  <input type="time" className="w-full pl-11 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-md text-slate-900 text-sm focus:outline-none transition-all" value={scheduledTime} onChange={(e)=> setScheduledTime(e.target.value)}/>
            ...
            <button onClick={handleSchedule} className="w-full flex items-center justify-center gap-2 py-3 rounded-md bg-slate-200 text-slate-700 hover:bg-brand hover:text-white transition">
