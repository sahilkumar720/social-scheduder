...
Platform
            ...
            <div className="flex flex-wrap gap-3">
              {PLATFORMS.map((p)=>{
                ...
                return(
                  ...
                  className={`flex items-center gap-1.5 p-3 rounded-md border transition-all duration-150 ${active ? "bg-light-back border-border1 text-brand scale-103" : "border-slate-200 text-slate-500 hover:border-slate-300"}`}>
          ...
          <div>
            ...
            <textarea required rows={5}  placeholder="What do you want to share today?" className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-slate-900 placeholder-slate-400 outline-none resize-none" value={content} onChange={(e)=>setContent(e.target.value)}/>
          ...
          <div>
            ...
            {mediaFile ? (
              ...
              <label className="flex items-center justify-center gap-2 py-10 border-2 border-dashed border-slate-200 rounded-xl cursor-pointer hover:border-brand hover:bg-red-50/30 transition-all group">
                <span className="text-sm text-slate-500 group-hover:text-brand transition-colors">Click to upload image and video </span>
    ...
    <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
      ...
      <div className="max-h-72 overflow-y-auto divide-y divide-slate-50">
        {scheduled.length === 0 ? (
         ...
         <div className="py-10 text-center text-slate-400 text-sm">No posts scheduled yet
          ...
          scheduled.map((post)=>(
            <div key={post._id} className="px-5 py-4 hover:bg-slate-50/60 transition-colors">
              ...
              <p className="text-sm text-slate-400 line-clamp-2 max-w-md">{post.content}
         ...
         <div className="py-10 text-center text-slate-400 text-sm">No published posts scheduled yet
          ...
          published.map((post)=>(
            <div key={post._id} className="px-5 py-4 hover:bg-slate-50/60 transition-colors">
              ...
              <p className="text-sm text-slate-400 line-clamp-2 max-w-4/5">{post.content}
