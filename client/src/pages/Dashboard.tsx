...
import { ActivityIcon, CheckCircle, CheckCircleIcon, ClockIcon, SendIcon, Share2Icon, TrendingUpIcon } from "lucide-react"
...
fetchDashboardData();
 ...
 const statCard = [ 
    {
        labal: "Scheduled Posts",
        value: stats.scheduled,
        icon: ClockIcon,
        trend: "+2 today",
    },
     {
        labal: "Published Posts",
        value: stats.published,
        icon: CheckCircleIcon,
        trend: "All time",
    },
     {
        labal: "Connected Accounts",
  ...
  return (
    <div className="space-y-8">
        ...
        <div>
            ...
            <p className="text-slate-500 text-sm mt-0.5"> Here's what's happening with your social account today. </p>
        </div>
...
{
    statCard.map((card)=>(
        <div key={card.labal} className="bg-white hover:bg-light-back relative border border-slate-200 rounded-2xl p-5 hover:border-border1 transition-all">
            <div className="flex items-center justify-between mb-4">
                <div className="text-3xl font-medium text-slate-800 tabular-nums">{card.value}</div>
                <div className="text-xs absolute right-4 top-4 text-brand flex items-center gap-1">
                    <TrendingUpIcon className="size-3"/>
                    {card.trend}
                </div>

            </div>
            <p className="text-sm text-slate-500 mt-1">{card.labal}</p>
